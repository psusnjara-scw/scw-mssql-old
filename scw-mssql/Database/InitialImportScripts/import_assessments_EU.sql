...
TRUNCATE TABLE import_assessments_EU
/*
CREATE TABLE Import_Assessments_EU (
	AssessmentDetails NVARCHAR(MAX)
);
*/

BULK INSERT dbo.import_assessments_EU
FROM 'E:\SCWDemo\MongoExportEU\scw_prod_alpha_game_013.assessments_EU.json' 
WITH (
    ROWTERMINATOR = '0x0a'
);


--SELECT * FROM dbo.import_assessments_EU
--
...
DECLARE JSONIMPORT CURSOR FOR
 ...
 FROM dbo.import_assessments_EU
...
SELECT StartDate_DEC FROM #Assessment
...
SELECT EndDate_DEC FROM #Assessment
...
SELECT InviteSendDate_DEC FROM #Assessment
